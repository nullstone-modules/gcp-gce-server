...
* `tcp` entries carry variant flags: `scheme` (`EXTERNAL` or `INTERNAL` passthrough) and `proxied` (global external proxy NLB with optional `proxy_protocol`, on a MIG named port). `http` entries carry `scope` and `scheme`; only `global` + `EXTERNAL_MANAGED` is implemented, other combinations fail the plan until gcp-network provides a proxy-only subnet. `app_metadata.subnet` is now provided for internal addresses.
