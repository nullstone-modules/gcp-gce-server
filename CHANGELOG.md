...
* `load_balancers` capability output is now a typed spec (`type = target_pool | tcp | http`; entries without `type` are target pools, so existing capabilities are unchanged). Server >= 0.1.0 is required for `gcp-gce-tcp-load-balancer` `mode = "backend_service"` and for `gcp-gce-http-load-balancer`.
